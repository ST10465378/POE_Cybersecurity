...
{//start of namespace
    ...
    {//start of class
        ...
        {//start of method
            ...
            Console.WriteLine("  ║         DynaBot Chat                 ║");
            Console.ForegroundColor = ConsoleColor.Cyan;
            Console.WriteLine("  ╚══════════════════════════════════════╝");
            Console.ResetColor();
            Console.WriteLine();

            welcome_and_username.TypeWrite("  Dynabot : Type your question, " + username + ". Type 'exit' to quit.", ConsoleColor.Blue);
            Console.WriteLine();
            ...
            while (true)
            {
                ...
                {
                    welcome_and_username.TypeWrite("  Dynabot : It looks like you didn't type anything. Could you rephrase?", ConsoleColor.Red);
                    continue;
                }
                ...
                {
                    Console.WriteLine();
                    welcome_and_username.TypeWrite("  Dynabot : Goodbye, " + username + "! Stay safe online. 🔒", ConsoleColor.Yellow);
                    Console.ForegroundColor = ConsoleColor.Cyan;
                    Console.WriteLine("");
                    Console.ResetColor();
                ...
                for (int i = 0; i < Questions.Length; i++)
                {
                    ...
                    {
                        welcome_and_username.TypeWrite("  Dynabot : " + Answers[i], ConsoleColor.Green);
                        foundAnswer = true;
                        break;
                    }
                }
                ...
                if (!foundAnswer)
                {
                    welcome_and_username.TypeWrite("  Dynabot : I didn't quite understand that. Could you rephrase?", ConsoleColor.Red);
                    welcome_and_username.TypeWrite("  Dynabot : Try asking about: passwords, phishing, or safe browsing.", ConsoleColor.Yellow);
                }
